function Convert-JsonObjectToJavaClass {
    ...
    process {
        ...
        [string]$timestamp = if ($ImportTimestamp) { $ImportTimestamp } else { (Get-Date).ToString("dd.MM.yyyy HH:mm") }
        ...
        Add-Member -InputObject $javaClass -Name "importTimestamp" -Value $timestamp -MemberType NoteProperty
